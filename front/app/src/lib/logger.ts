...
export class ConsoleLogger implements Logger {
  ...
  debug(...messages: [string, ...any]) {
    const [message, ...args] = messages;
    if(this.#level >= LogLevel.DEBUG) {
      console.log(`${this.#ts()}${color.blue(`DEBUG`)}${this.#s} ${message}`, ...args)
    }
  }

  info(...messages: [string, ...any]) {
    const [message, ...args] = messages;
    if(this.#level >= LogLevel.INFO) {
      console.log(`${this.#ts()}${color.green(`INFO`)} ${this.#s} ${message}`, ...args);
    }
  }

  warn(...messages: [string, ...any]) {
    const [message, ...args] = messages;
    if(this.#level >= LogLevel.WARN) {
      console.warn(`${this.#ts()}${color.yellow(`WARN`)} ${this.#s} ${message}`, ...args)
    }
  }
