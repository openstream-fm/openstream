<script lang="ts">
	...
	const mdiAdMobViewBox = "-3 -3 29 29";
	...
	const _validate_admob_app_id = (value: string | null): string | null => {
		...
		if(!/^ca\-app\-pub\-[0-9]{16}\~[0-9]{10}$/.test(value)) {
			// TODO: locale
			return "AdMob app ID doesn't match the pattern";
		}
		return null;
	}
	...
	const _validate_admob_banner_id = (value: string | null): string | null => {
		...
		if(!/^ca\-app\-pub\-[0-9]{16}\/[0-9]{10}$/.test(value)) {
			// TODO: locale
			return "AdMob banner ID doesn't match the pattern";
		}
		return null;
	}
</script>
...
<div class="section section-advanced">
	...
	<div class="fields section-advanced-fields" class:open={advanced_open} use:display_fly_enter={{ show: advanced_open, start: false, duration: 300, y: -25 }}>
		...
		<div class="field">
			<!-- TODO: locale -->
			<NullTextField icon={mdiAdMob} icon_viewbox={mdiAdMobViewBox} trim label="AdMob app ID" bind:value={current.user_metadata.mob_app.admob_app_id} />
			<Validator value={current.user_metadata.mob_app.admob_app_id} fn={_validate_admob_app_id} />
		</div>

		<div class="field">
			<!-- TODO: locale -->
			<NullTextField icon={mdiAdMob} icon_viewbox={mdiAdMobViewBox} trim label="AdMob banner ID" bind:value={current.user_metadata.mob_app.admob_banner_id} />
