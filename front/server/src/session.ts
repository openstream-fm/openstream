...
const get_cookie_session = (req: Request, logger: Logger): SessionData => {
  try {
    ...
    try {
      ...
      logger.warn(`json parse error: JSON.parse('${value}'): ${e}`)
    }
...
export const session = (config: Config, _logger: Logger) => {
  ...
  const options = { maxAge, httpOnly: true,  sameSite: "strict" as const, signed: true, domain: config.session.domain };
  const clear_options = { httpOnly: options.httpOnly, sameSite: options.sameSite, domain: options.domain, signed: true };

  const router = Router();
  router.use(cookieParser(config.session.secret));
  router.use((req: Request, res: Response, next: NextFunction) => {
    req.cookie_session = get_cookie_session(req, logger);
    res.set_session = (data: SessionData) => {
      //const encoded = encrypt(JSON.stringify(data), key, logger);
      res.cookie(name, JSON.stringify(data), options);
    }
