...
fn with_ms_precision(date: chrono::DateTime<Utc>) -> chrono::DateTime<Utc> {
  bson::DateTime::from_millis(date.timestamp_millis()).into()
}

pub fn now() -> chrono::DateTime<Utc> {
  bson::DateTime::now().into()
}
...
pub fn serialize<S: Serializer>(date: &chrono::DateTime<Utc>, ser: S) -> Result<S::Ok, S::Error> {
  if ser.is_human_readable() {
    ...
    Serialize::serialize(&with_ms_precision(*date), ser)
  } else {
...
pub fn deserialize<'de, D: Deserializer<'de>>(de: D) -> Result<chrono::DateTime<Utc>, D::Error> {
  if de.is_human_readable() {
    ...
    Deserialize::deserialize(de).map(with_ms_precision)
  } else {
...
pub mod option {
  ...
  pub fn serialize<S: Serializer>(
    ...
    if ser.is_human_readable() {
      ...
      opt.map(with_ms_precision).serialize(ser)
    } else {
