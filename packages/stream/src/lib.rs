...
use hyper::{Server, StatusCode, Body, header::CONTENT_TYPE, http::HeaderValue};
use prex::{Request, Next, Response};
...
async fn handle(req: Request, _next: Next) -> Response {
    ...
    let mut stream = match channels::subscribe(id) {
        ...
        None => {
            ...
            *res.body_mut() = Body::from(format!("Stream with id {id} is not actively streaming right now"));
            return res;
        }
    };
    ...
    res.headers_mut().insert(CONTENT_TYPE, HeaderValue::from_static("audio/mpeg"));
