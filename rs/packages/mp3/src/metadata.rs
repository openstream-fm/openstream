    ...
    *i = x as u32 + tag_size as u32;
    ...
    loop {
      ...
      match frame_name {
        ...
        // ----- TEXT FRAMES -----
        // -----------------------
        b"TALB" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.album_movie_show),
        ...
        b"TDLY" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.playlist_delay),
        b"TENC" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.encoded_by),
        b"TEXT" => get_text_fields(buf, pos, frame_size, &mut changes, &mut op.text_writers),
        ...
        b"TIT" | b"TIT2" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.title),
        ...
        b"TOPE" => get_text_fields(buf, pos, frame_size, &mut changes, &mut op.original_artists),
        ...
        b"TPOS" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.part_of_a_set),
        ...
        b"TRDA" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.recording_dates),
        ...
        b"IPLS" => get_text_field(buf, pos, frame_size, &mut changes, &mut op.involved_people),
        // ----------------------
        // ----- URL FRAMES -----
        // ----------------------
        ...
        b"WOAF" => get_url_field(buf, pos, frame_size, &mut changes, &mut op.official_webpage),
...
fn read_header(buf: &[u8], i: &mut u32, meta: &mut MP3Metadata) -> Result<bool, Error> {
    ...
    && header & (3 << 10) != (3 << 10)
  {
    ...
    frame.bitrate = BITRATES[get_line(frame.version, frame.layer)][((header >> 12) & 0xF) as usize];
    frame.sampling_freq =
