...
pub struct Builder {
  ...
  writer_err: writer::Builder,
  writer_out: writer::Builder,
  format: fmt::Builder,
  built: bool,
}
...
impl Default for Builder {
  fn default() -> Self {
    ...
    let mut writer_err = writer::Builder::default();
    writer_err.target(Target::Stderr);

    let mut writer_out = writer::Builder::default();
    writer_out.target(Target::Stdout);

    let format = fmt::Builder::default();
    let built = false;
...
impl Builder {
  ...
  /// let mut builder = Builder::new();
  ///
  /// builder.out_target(Target::Stdout);
  /// ```
  pub fn out_target(&mut self, target: fmt::Target) -> &mut Self {
    self.writer_out.target(target);
    self
  }

  /// sets the WARN and ERROR level target
  pub fn err_target(&mut self, target: fmt::Target) -> &mut Self {
    self.writer_err.target(target);
    self
  }
  ...
  pub fn write_style(&mut self, write_style: fmt::WriteStyle) -> &mut Self {
    self.writer_out.write_style(write_style);
    self.writer_err.write_style(write_style);
    self
  }
  ...
  pub fn parse_write_style(&mut self, write_style: &str) -> &mut Self {
    self.writer_out.parse_write_style(write_style);
    self.writer_err.parse_write_style(write_style);
    self
  }
  ...
  pub fn is_test(&mut self, is_test: bool) -> &mut Self {
    self.writer_err.is_test(is_test);
    self.writer_out.is_test(is_test);
    self
  }
  ...
  pub fn build(&mut self) -> Logger {
    ...
    Logger {
      writer_out: self.writer_out.build(),
      writer_err: self.writer_err.build(),
...
impl Logger {
  ...
  fn writer_for(&self, level: log::Level) -> &Writer {
    match level {
      log::Level::Error | log::Level::Warn => &self.writer_err,
      log::Level::Info | log::Level::Debug | log::Level::Trace => &self.writer_out,
    }
  }
}
    ...
    if self.matches(record) {
      ...
      let print = |formatter: &mut Formatter, record: &Record| {
        let _ = (self.format)(formatter, record)
          .and_then(|_| formatter.print(self.writer_for(record.level())));
  ...
  impl fmt::Debug for Builder {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
      if self.built {
        ...
        f.debug_struct("Logger")
          .field("filter", &self.filter)
          .field("writer_out", &self.writer_out)
          .field("writer_err", &self.writer_err)
          .finish()
      }
    }
  }
