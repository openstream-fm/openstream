...
impl StreamServer {
  ...
  pub fn start(
    self,
  ) -> Result<impl Future<Output = Result<(), std::io::Error>> + 'static, StreamServerError> {
    ...
    for addr in self.addrs.iter().copied() {
      ...
      let server = Server::new(tcp, app.clone(), self.shutdown.signal());
      ...
      {
        use owo_colors::*;
        info!("stream-alt server bound to {}", addr.yellow());
      }
