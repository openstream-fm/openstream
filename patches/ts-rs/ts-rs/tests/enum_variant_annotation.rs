...
fn test_enum_variant_rename_all() {
    assert_eq!(
        A::inline(),
        "{ MESSAGE_ONE: { sender_id: string, number_of_snakes: number, } } | { MESSAGE_TWO: { senderId: string, numberOfCamels: number, } }",
    );
}
...
fn test_enum_variant_rename() {
    assert_eq!(
        B::inline(),
        "{ SnakeMessage: { sender_id: string, number_of_snakes: number, } } | { CamelMessage: { sender_id: string, number_of_camels: number, } }",
    );
}
